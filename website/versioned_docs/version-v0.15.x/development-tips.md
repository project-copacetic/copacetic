...
## Run Debian and Ubuntu real-image tests

Changes to native Chisel, apt-less full-status, or Distroless status-directory
patching should run the digest-pinned real-image suite in addition to `make test`.
The suite requires Docker with the containerd image store, Docker Buildx, Trivy,
and public network access. Build Copa, pull the pinned Chisel tooling image, and
...
Ubuntu and Debian archives, so they are not part of `make test`. The complete
fixture matrix and assertions are documented in
[`test/e2e/dpkg/README.md`](https://github.com/project-copacetic/copacetic/blob/main/test/e2e/dpkg/README.md).
