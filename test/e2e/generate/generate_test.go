...
const (
	nginxImage = "nginx:1.25.3-bookworm"
)
...
func TestGenerateWithReport(t *testing.T) {
	...
	patchedImage := "nginx:1.25.3-bookworm-patched-test"
...
func TestGenerateWithoutReport(t *testing.T) {
	...
	testImage := "nginx:1.25.3-bookworm"
...
func TestGenerateToStdout(t *testing.T) {
	...
	patchedImage := "nginx:1.25.3-bookworm-piped-test"
...
func TestGenerateWithVEXOutput(t *testing.T) {
	...
	// build the image using docker
	patchedImage := "nginx:1.25.3-bookworm-vex-patched-test"
	buildCmd := exec.Command("docker", "build", "-t", patchedImage, buildContextDir)
	buildOutput, err := buildCmd.CombinedOutput()
	require.NoError(t, err, fmt.Sprintf("failed to build patched image: %s", string(buildOutput)))
