...
var DownloadChart = func(ctx context.Context, name, version, repository string) (*helmchart.Chart, error) {
...
var RenderChart = func(ctx context.Context, ch *helmchart.Chart) (string, error) {
	settings := helmcli.New()
	cfg := &helmaction.Configuration{}
	...
	release, err := install.RunWithContext(ctx, ch, map[string]interface{}{})
	if err != nil {
		return "", fmt.Errorf("failed to render chart '%s': %w", ch.Metadata.Name, err)
	}

	var rendered strings.Builder
	rendered.WriteString(release.Manifest)
	for _, hook := range release.Hooks {
		rendered.WriteString("\n---\n")
		rendered.WriteString(hook.Manifest)
	}
...
// DiscoverChartImages renders and extracts all container images from a Helm chart.
// Overrides are applied to the discovered images.
func DiscoverChartImages(ctx context.Context, ch *helmchart.Chart, overrides map[string]OverrideSpec) ([]ChartImage, error) {
