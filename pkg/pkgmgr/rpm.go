...
// to confirm it exists. If it doesn't, return an error so we can fallback.
...
func (rm *rpmManager) probeRPMStatus(ctx context.Context, toolImage string) error {
	...
	err = toolingBase.Validate(ctx, &llb.Constraints{})
	if err != nil {
		log.Errorf("unable to validate tooling image %s", toolImage)
	}
