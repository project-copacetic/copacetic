...
func TestOptimalVersionSelection(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			expected:         "1.26.19", // Should pick highest patch version that fixes
		...
		},
		{
			...
			expected:         "1.26.19", // Should pick smallest minor version in same major
		...
		},
		{
			...
			expected:         "2.0.6", // Should pick smallest major version when no other option
			description:      "Should pick smallest major version when no other choice",
		},
		{
			name:             "comma_separated_versions",
			installedVersion: "1.26.16",
			fixedVersions:    []string{"2.0.7, 1.26.18"},
			expected:         "1.26.18", // Should handle comma-separated and pick patch version
			description:      "Should handle comma-separated versions correctly",
		},
	}
