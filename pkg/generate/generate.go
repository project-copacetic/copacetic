...
func generateWithContext(
	...
	patchLayer, err := extractPatchLayer(ctx, ch, bkClient, ref, patchedImageName, updates, workingFolder, scanner, ignoreErrors, reportFile, opts.Format, opts.Output, progress)
	if err != nil {
		return err
	}
...
func extractPatchLayer(
	...
	eg.Go(func() error {
		...
		if buildErr == nil && reportFile != "" && validatedManifest != nil && output != "" {
			...
			if len(validatedManifest.Updates) > 0 {
	...
	if err := eg.Wait(); err != nil {
		...
		if updates == nil && (strings.Contains(err.Error(), "grep -q upgradable") || strings.Contains(err.Error(), "apk list")) && strings.Contains(err.Error(), "exit code: 1") {
			...
			// Create a minimal tar with just an empty file
			emptyTarData := createEmptyTar()
...
// createEmptyTar creates a minimal tar with an empty file to represent no changes
func createEmptyTar() []byte {
	buf := &bytes.Buffer{}
	tw := tar.NewWriter(buf)
	
	// Add a minimal empty file to create a valid tar
	hdr := &tar.Header{
		Name: ".empty",
		Mode: 0o644,
		Size: 0,
